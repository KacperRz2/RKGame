...
extern inline float sine(float a){
    ...
    // unsigned int angle_indx = (unsigned int)(a * tmp);
	// if(angle_indx < 0) exit(0); 
	// if(angle_indx >= ANGLE_PARTS) exit(0);
...
extern inline float SineSafe(float a){
    static const float tmp = (float)ANGLE_PARTS * 0.5F / SDL_PI_F;
    // unsigned int angle_indx = (unsigned int)(a * tmp) % ANGLE_PARTS;
	// if(angle_indx < 0) angle_indx += ANGLE_PARTS; 
	// if(angle_indx >= ANGLE_PARTS) angle_indx -= ANGLE_PARTS;
    // if(angle_indx >= ANGLE_PARTS) exit(0);
